...
export default function PasswordChangeForm({ pwdOk, pwdError, embedded, redirectAfter = '/dashboard/profile' }: Props) {
  ...
  return (
    <form action={updatePassword} className={formClass}>
      ...
      {pwdOk && (
        <p className="rounded-lg border border-[#bfe7d1] bg-[#e8faf0] px-3 py-2 text-sm text-[#0d9f6e]">
          Пароль обновлён
        </p>
      )}
      {pwdError && ERR[pwdError] && (
        <p className="rounded-lg border border-[#f3c5c7] bg-[#fdecec] px-3 py-2 text-sm text-[#e5484d]">
          {ERR[pwdError]}
        </p>
      )}
      <div>
        <label className="mb-1 block text-sm text-[#6b6b80]">Новый пароль</label>
        <input
          name="password"
          type="password"
      ...
      <div>
        <label className="mb-1 block text-sm text-[#6b6b80]">Повторите пароль</label>
        <input
      ...
      <button type="submit" className="rounded-lg bg-[#1a1a2e] px-4 py-2 text-sm font-medium text-white hover:bg-[#10101f]">
        Сменить пароль
      </button>
    </form>
  )
