...
export default function AuthLanding() {
  ...
  // When GIS is ready and container is mounted — render Google's button into it
  useEffect(() => {
    ...
    window.google?.accounts?.id?.renderButton(gisContainerRef.current, {
      ...
      text: 'signin_with',
      shape: 'rectangular',
      width: String(gisContainerRef.current.offsetWidth || 340),
    })
  }, [gisRendered])
          ...
          <main className="auth-main">
            <div className="auth-card">
              ...
              <div className="auth-social">
                {/* GIS renderButton — shows when script loaded; opens real popup, works in Russia */}
                {gisRendered ? (
                  <div style={{ position: 'relative', minHeight: 42 }}>
                    {oauthLoading === 'google' && (
                      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.8)', zIndex: 2, borderRadius: 12, fontSize: 14, color: '#6b7280', gap: 8 }}>
                    ...
                    <div ref={gisContainerRef} style={{ lineHeight: 0 }} />
                  </div>
                ) : (
                  <button
                    type="button"
                    ...
                    onClick={() => void handleOAuthRedirect('google')}
                  >
